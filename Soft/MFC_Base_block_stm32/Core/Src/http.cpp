...
const char main_page_name[] = {"main_page.html"};
...
void tcp_send_http_one_data() {
	...
	if (httpsockprop[tcpprop.cur_sock].http_doc == EXISTING_HTML) {
		...
		for (i = 0; i <= num_sect; i++) {
			...
			// memcpy(sect + 3, pages_map[httpsockprop[tcpprop.cur_sock].fname] + i * 512, 512); // reading 512 byte sectors from map and writing them to tx
			// buff
...
void http_request() {
	...
	if (RXbyte == (uint8_t)' ') {
		...
		is_page_exist = (!strcmp(httpsockprop[tcpprop.cur_sock].fname, main_page_name));
